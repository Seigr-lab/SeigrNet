...
async function main() {
  ...
  try {
    ...
    const notebooks=['index','concepts','seigrasm','silicon','experiments','evidence','method','electrical','stability','repeatability','freshness','transitions','held','adaptive','guide'];
    for(const width of [1440,390]){
      ...
      for(const file of ['index.html','html/insektsreservat.html','html/sound.html','html/manifesto.html',...notebooks.map(n=>'lab/hyphos/'+n+'.html')]){
        ...
        if(['index.html','lab/hyphos/index.html','lab/hyphos/experiments.html','lab/hyphos/adaptive.html','html/sound.html'].includes(file))await screenshot(file.replaceAll('/','-').replace('.html','')+'-'+width);
      }
    }
    await go('lab/hyphos/experiments.html');
    await check('seven experiments',"document.querySelectorAll('.study-card').length===7");
